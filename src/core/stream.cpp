...
QList<StreamObject::Data::Format> StreamObject::Data::defaultFormats() const
{
    ...
    StreamFormat bestAudioFormat;
    bool hasAudioFormat = false;
    auto audioList = audioFormats();
    if (!audioList.isEmpty()) {
        // formats are ordered from worst to best quality, so last is best
        bestAudioFormat = audioList.last();
        hasAudioFormat = true;
    }
    ...
    for (auto format : formats) {
        if (format.hasVideo()) {
            if (format.hasMusic()) {
                ...
                // Combine with the best audio format
                StreamFormat mergedFormat = format;
                mergedFormat.formatId = format.formatId + bestAudioFormat.formatId;
                mergedFormat.acodec = bestAudioFormat.acodec;
                mergedFormat.abr = bestAudioFormat.abr;
                mergedFormat.asr = bestAudioFormat.asr;
                mergedFormat.formatNote = QString("%1 + %2").arg(format.formatNote, bestAudioFormat.formatNote);

                auto unique_sort_identifier = QString("%0 %1 %2")
                        .arg(mergedFormat.width, 16, 10, QChar('0'))
                        .arg(mergedFormat.height, 16, 10, QChar('0'))
                        .arg(mergedFormat.toString());

                map.insert(unique_sort_identifier, mergedFormat);
            }
        }
    }
    return map.values();
